...
gridsizeinkm <- 30
cellsize <- (lat_dataset[2] - lat_dataset[1]) * gridsizeinkm * .0018
...
The number of `r summarize_list(obs_filter[-1])` in Norway per `r gridsizeinkm` x `r gridsizeinkm` km cs cells, versus the human population in those cells. R² of the linaer regression is `r summary(lm(num ~ TOT_P, data=num_vs_pop))$r.squared`.
...
```{r relative_to_lm_plot_rasterized, messages=FALSE, echo=FALSE}

B0 <- (summary(lm(num ~ TOT_P, data=num_vs_pop))$coefficients)["(Intercept)","Estimate"]
B1 <- (summary(lm(num ~ TOT_P, data=num_vs_pop))$coefficients)["TOT_P","Estimate"]

num_vs_pop$relativeToLM <- num_vs_pop$num / (B0 + B1 * num_vs_pop$TOT_P) 
...
The log number of `r summarize_list(obs_filter[-1])` observations in Norway divided by the number predicted by the regression line (numberOfObservations = `r B0` + `r B1` * humanPopulation),  within `r gridsizeinkm` x `r gridsizeinkm` km cells.
